...
test("ChromeTabLedger recycles owned idle tabs without closing user tabs", async () => {
  ...
  assert.equal(productTab.id, "owned-3");
  assert.deepEqual(browser.closedTabs, ["owned-2"]);
  ...
  assert.deepEqual(new Set(browser.closedTabs), new Set(["owned-1", "owned-2", "owned-3"]));
  assert.ok(!browser.closedTabs.includes("preexisting-user-tab"));
});
